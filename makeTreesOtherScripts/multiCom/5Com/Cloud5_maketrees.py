...
def makeStarting(tips,RF_norm_start,name,RF_norm_cloud,cloud_size):
	...
	# do it all again with another starting tree
	t5 = readTree.NNI_mult_moves(in_tree=t1,num_moves=NNI_moves_start,node_choice='random',no_dup_start_tree='F', req_min_RF=RF_norm_start)
	# now we need to check d24 and d34
	...
	while d25 != RF_norm_start or d35 != RF_norm_start or d45 != RF_norm_start:
		print("redoing fifth start tree")
		t5 = readTree.NNI_mult_moves(in_tree=t1,num_moves=NNI_moves_start,node_choice='random',no_dup_start_tree='F', req_min_RF=RF_norm_start)
		d25=readTree.rf_unweighted(t2,t5,"T")[1]
		d35=readTree.rf_unweighted(t3,t5,"T")[1]
		d45=readTree.rf_unweighted(t3,t5,"T")[1]
		print(str(d25)+", "+str(d35)+", "+str(d45))

...
def makeStarting(tips,RF_norm_start_far,RF_norm_start,name,RF_norm_cloud,cloud_size):
	...
	print("RF start: " + str(RF_norm_start) + "  RF start far: " + str(RF_norm_start_far))
	...
	print("NNI: " + str(NNI_moves_start) + "  NNI far: " + str(NNI_moves_start_far))
	...
	while round(d24, 2) != RF_norm_start or round(d34, 2) != RF_norm_start:
		print("redoing fourth start tree - "+name)
		print(round(d24, 2),round(d34, 2),RF_norm_start)
		t4 = readTree.NNI_mult_moves(in_tree=t1,num_moves=NNI_moves_start,node_choice='random',no_dup_start_tree='F', req_min_RF=RF_norm_start)
		d24=readTree.rf_unweighted(t2,t4,"T")[1]
		d34=readTree.rf_unweighted(t3,t4,"T")[1]
	print("t1 to t4 RF - "+str(round(d14,2)))
	print("t2 to t4 RF - "+str(round(d24,2)))
	print("t3 to t4 RF - "+str(round(d34,2)))

	# do it all again with another starting tree
	t5 = readTree.NNI_mult_moves(in_tree=t1,num_moves=NNI_moves_start_far,node_choice='random',no_dup_start_tree='F', req_min_RF=RF_norm_start_far)
	# now we need to check d24 and d34
	d15=readTree.rf_unweighted(t1,t5,"T")[1]
	d25=readTree.rf_unweighted(t2,t5,"T")[1]
	d35=readTree.rf_unweighted(t3,t5,"T")[1]
	d45=readTree.rf_unweighted(t4,t5,"T")[1]
	print("first t1 to t5 RF - "+str(round(d15,2)))
	print("first t2 to t5 RF - "+str(round(d25,2)))
	print("first t3 to t5 RF - "+str(round(d35,2)))
	print("first t4 to t5 RF - "+str(round(d45,2)))

	while round(d25, 2) != RF_norm_start_far or round(d35, 2) != RF_norm_start_far or round(d45, 2) != RF_norm_start_far:
		print("redoing fifth start tree, Far Tree - "+name)
		t5 = readTree.NNI_mult_moves(in_tree=t1,num_moves=NNI_moves_start_far,node_choice='random',no_dup_start_tree='F', req_min_RF=RF_norm_start_far)
		d25=readTree.rf_unweighted(t2,t5,"T")[1]
		d35=readTree.rf_unweighted(t3,t5,"T")[1]
		d45=readTree.rf_unweighted(t3,t5,"T")[1]
	print("t1 to t5 RF - "+str(round(d15,2)))
	print("t2 to t5 RF - "+str(round(d25,2)))
	print("t3 to t5 RF - "+str(round(d35,2)))
	print("t4 to t5 RF - "+str(round(d45,2)))
	...
	readTree.write_single_tree(t5,'%s_starting_tree_5.tree' % name)

	# Write out log file
	rfs=str(d12)+", "+str(d13)+", "+str(d23)+", "+str(d14)+", "+str(d24)+", "+str(d34)+", "+str(d15)+", "+str(d25)+", "+str(d35)+", "+str(d45)
	with open('%s.log' % name, "w") as log_file:
		line1 = "File name: "+str(name)
		line1b = "Tips: "+str(tips)+", Trees per cloud: "+str(cloud_size)
		line2 = "Starting trees - RF_input: "+str(RF_norm_start)+"RF_input_far: "+str(RF_norm_start_far)+", RF_calc: "+str(rfs)+", NNI_moves: "+str(NNI_moves_start)+", NNI_moves_far: "+str(NNI_moves_start_far)
...
def makeCloud(tips,RF_norm_cloud,name,cloud_size,starting_trees): 
	...
	cluster5 = readTree.NNI_mult_trees(in_tree=t5,num_out_trees=c_size,num_nni_moves=NNI_moves_cloud,out='list')
	tree_list=[cluster1,cluster2,cluster3,cluster4,cluster5]
...
def main():
	...
	RF_norm_start_far = 1.0
	RF_norm_start = 0.74
	start_name = str(RF_norm_start_far)+"v"+str(RF_norm_start)
	# Make a bunch of trees
	for num in range(1,number_replicates+1):
		name =("%stip_%strees_%s_%sstart_%s" % (tips,cloud_size,RF_norm_cloud,start_name, num))
		trees = makeStarting(tips,RF_norm_start_far,RF_norm_start,name,RF_norm_cloud,cloud_size)
